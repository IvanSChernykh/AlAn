...
result = {"block": [], "block_type": [], "seq_num": [], "block_len": [], "ori": [], "start": [], "stop": [], "len": [], "core_start": [], "core_stop": [], "core_len": [], "genes_num": [], "is_inside": [], "core_%": [], "is_ideal": []}
...
for block_ori, loc_table in table.groupby(["npg_block", "npg_block_ori"]):
    ...
    for starts_stops_counter in range(len(starts_stops)):
        if starts_stops[starts_stops_counter][1]:
            ...
            for seq in seqs:
                result[seq].append(seq in group_table["sequence"].unique())
            last_coord = -1
            try:
            ...
            result["is_ideal"].append((core_percent/100 >= core_percent_threshold) and inside)
groups = pd.DataFrame(result)
if out_excel:
    groups.to_excel(f"{outdir}/groups.xlsx")
else:
    groups.to_csv(f"{outdir}/groups.csv")
result_no_ori = {"block": [], "block_type": [], "seq_num": [], "block_len": [], "ori": [], "start": [], "stop": [], "len": [], "core_start": [], "core_stop": [], "core_len": [], "genes_num": [], "is_inside": [], "core_%": [], "is_ideal": []}
for seq in seqs:
    result_no_ori[seq] = []
for block, loc_table in table.groupby("npg_block"):
    ...
    for starts_stops_counter in range(len(starts_stops)):
        if starts_stops[starts_stops_counter][1]:
            ...
            for seq in seqs:
                result_no_ori[seq].append(seq in group_table["sequence"].unique())
            last_coord = -1
            try:
            ...
            result_no_ori["is_ideal"].append((core_percent/100 >= core_percent_threshold) and inside)
